...
describe("diary date stamp", () => {
  ...
  it("fixes obvious clinical typos in the preview stamp", () => {
    ...
    expect(fixObviousTypos("Синдром психомотоной расторможенности")).toBe(
...
describe("тире в готовом дневнике", () => {
  it("показывает короткие тире вместо длинных", () => {
    const out = applyDiaryStamp(
      "Психический статус: Настроение ровное — без колебаний. Т – 36,6 С.",
      { title: "День 6 · 14.09.2026 · Ежедневный осмотр" },
    );
    expect(out).not.toMatch(/[—–]/);
    expect(out).toContain("ровное - без колебаний");
  });
});
