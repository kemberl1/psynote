/** Очевидные опечатки модели в клинических формулировках. */

const OBVIOUS_TYPOS: ReadonlyArray<readonly [string, string]> = [
  ["Сидрос", "Синдром"],
  ["сидрос", "синдром"],
  ["СИДРОС", "СИНДРОМ"],
  ["психмоторн", "психомоторн"],
  ["Психмоторн", "Психомоторн"],
  ["ПСИХМОТОРН", "ПСИХОМОТОРН"],
  ["психомотон", "психомоторн"],
  ["Психомотон", "Психомоторн"],
  ["ПСИХОМОТОН", "ПСИХОМОТОРН"],
  ["убгает", "убегает"],
  ["самостоятельно не предъявляет", "не предъявляет"],
  ["Самостоятельно не предъявляет", "не предъявляет"],
  ["самостоятельно не формирует", "не предъявляет"],
  ["Самостоятельно не формирует", "не предъявляет"],
  ["mildly", "слегка"],
  ["Mildly", "Слегка"],
  ["slightly", "слегка"],
  ["Slightly", "Слегка"],
  ["на оклики", "на замечания"],
  ["На оклики", "На замечания"],
];

const ILLEGAL_HOLD =
  /(?:в такие моменты\s+)?(?:требуется\s+)?физическ[а-яё]*\s+удержани[еяю](?:\s+и\s+помощь\s+персонала)?/gi;

const STAFF_HOLD = /удержива[а-яё]{0,12}\s+с\s+помощью\s+персонала/gi;
const HOLD_ATTEMPT = /при попытке удержани[а-яё]*/gi;
const HOLD_BY_HAND = /при удержании за руку/gi;

/** Заменяет только однозначные опечатки вроде «Сидрос» → «Синдром». */
export function fixObviousTypos(text: string): string {
  if (!text) return text;
  let out = text;
  for (const [from, to] of OBVIOUS_TYPOS) {
    if (out.includes(from)) out = out.split(from).join(to);
  }
  out = out.replace(
    ILLEGAL_HOLD,
    "применена мягкая фиксация конечностей под контролем медперсонала",
  );
  out = out.replace(HOLD_ATTEMPT, "при попытке остановить");
  out = out.replace(HOLD_BY_HAND, "если взять за руку");
  out = out.replace(STAFF_HOLD, "удерживается");
  out = out.replace(/[ \t]{2,}/g, " ");
  return out;
}
