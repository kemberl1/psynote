...
Результат можно **скачать в Word/PDF** или **скопировать текстом**.
...
flowchart LR
    ...
    E --> F[Готовый дневник: скачать Word или PDF или скопировать]
```
...
| **LLM** | **Корпоративный API (X5 CoPilot, OpenAI-совместимый)**: основная модель `x5-airun-large`, fallback на `x5-airun-medium`/`small` | Способ подключения подтверждён по референс-проекту `hh_analyser`: base URL, Bearer-ключ, корпоративный CA-сертификат. Автоматический fallback при сбое основной модели. |
...
**Делаем сейчас:** только **страницу генерации дневников** (ежедневный + раз в 10 дней), с опросником, анонимизацией, RAG, генерацией, экспортом, аутентификацией и историей запросов.
