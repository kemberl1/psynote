...
- **Состояние «Результат»**: сгенерированный дневник + панель действий (копировать, Word, редактировать).
...
- Панель действий: **Копировать** · **Скачать Word** · **Редактировать**.
...
- **Экспорт пакета:** кнопка «Экспорт в Word» на экране результата → `POST /api/v1/export/batch` (все успешные дни в один файл, по дате).

### UX

- Превью перед запуском: «N ежедневных, M осмотров».
- Прогресс: список дней со статусами (ожидание / … / ✓ / ✗).
- По завершении — боковой список дней + просмотр текста; кнопка **«Экспорт в Word»** скачивает все успешные дни одним файлом; записи доступны в истории (экспорт по одному — как на `/diary`).
...
| Генерация идёт | стриминг текста + индикатор модели («deepseek-v4-flash») |
