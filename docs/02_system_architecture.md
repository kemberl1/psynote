...
| **API-Gateway + Go-сервисы** | **Go** (net/http или chi/fiber, gRPC опц.) | Аутентификация (JWT), маршрутизация, **анонимизация (гейт)**, оркестрация RAG↔LLM, **fallback между моделями**, сборка Word, запись обезличенной истории. |
...
3. **Экспорт в Word** — CPU-bound потоковая генерация файлов; Go отлично держит нагрузку и легко стримит результат.
...
| Экспорт документов | **Go: прямая сборка DOCX (OOXML)** | генерация на фронте (хуже контроль над форматом и стилем). |
