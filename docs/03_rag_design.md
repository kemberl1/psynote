...
> Важно: в референс-проекте `hh_analyser` есть и **корпоративная эмбеддинг-модель** `x5-airun-embed-4b` (Qwen3-Embedding-4B). Но она **облачная**, поэтому для индексации медицинского корпуса мы её **не используем** (приватность). Локальная модель — осознанный выбор ради NFR-P3.

| Критерий | Локальная e5-large | Облачная x5-airun-embed-4b |
|---|---|---|
...
## 9. Подключение LLM (по референс-проекту `hh_analyser`)

> Изучен проект `/Users/user/Desktop/hh_analyser`. Ниже — как именно подключается корпоративный LLM, чтобы повторить это в нашем Go-Gateway (или, как «План Б», в Python).

### 9.1. Базовые параметры (подтверждены в `hh_analyser`)
| Параметр | Значение | Источник |
|---|---|---|
| Тип API | **OpenAI-совместимый** (X5 CoPilot API 2.0) | `x5_copilot.py` использует `openai` SDK |
| `base_url` | `https://api-copilot.x5.ru/aigw/v1/` | `config.py` → `LLM_BASE_URL` |
| Авторизация | **Bearer-ключ** в заголовке (`API_KEY`) | `config.py` → `API_KEY` |
| Эндпоинты | `POST /chat/completions`, `POST /embeddings`, `GET /models`, `GET /api_key/info` | docs `hh_analyser` |
| TLS / сертификаты | **Внутренний корпоративный CA X5** (sre-vault.x5.ru), которого нет в публичном `certifi`. Решается через **кастомный CA-bundle** (PEM), верификация TLS **остаётся включённой** | `config.py` → `LLM_CA_BUNDLE`, монтируется в образ `/app/certs/x5_root_ca.pem` |
| Тайм-ауты/ретраи | timeout 60s, до 3 ретраев с экспоненциальным backoff; 401/403 — не ретраим, 429/5xx/timeout — ретраим | `x5_copilot.py` (tenacity) |

### 9.2. Как именно подключаются сертификаты (важно для воспроизведения)
В `hh_analyser` это сделано так (паттерн для копирования):

- В `.env`: `LLM_CA_BUNDLE=/app/certs/x5_root_ca.pem`.
- PEM с корпоративным CA **вшит в образ** (`/app/certs/...`) и/или монтируется volume.
- Клиент строит TLS-контекст, доверяющий этому CA, **не отключая проверку** (`ssl.create_default_context(cafile=ca_bundle)`).
...
### 9.3. Доступные модели (из docs `hh_analyser`)
| Модель | Назначение | Используем |
|---|---|---|
| `x5-airun-large` | крупная, для сложных/научных текстов | ✅ **основная для генерации** |
| `x5-airun-medium` | чат, адаптирована под русский | ✅ **fallback #1** (и дёшево/быстро) |
| `x5-airun-small` | простые задачи | ✅ **fallback #2** |
| `x5-airun-embed-4b` | эмбеддинги (облако) | ❌ (мы считаем эмбеддинги локально) |

> Перед стартом нужно подтвердить доступность моделей нашему токену через `GET /models` (как делали в `hh_analyser` на Phase 6.5). Если `x5-airun-large` недоступна токену — основной становится `medium`.

### 9.4. Абстракция и заменяемость
Повторяем принцип `hh_analyser`: **абстрактный интерфейс LLM-клиента** (методы `Chat`, при необходимости `Embeddings`) + конкретная реализация для X5. Это позволяет заменить провайдера, не трогая бизнес-логику.
...
```mermaid
flowchart TB
    Start[Запрос на генерацию] --> P[Попытка: x5-airun-large]
    P -->|успех| OK[Ответ]
    P -->|ошибка 5xx / timeout / недоступна| M[Попытка: x5-airun-medium]
    M -->|успех| OK
    M -->|ошибка| S[Попытка: x5-airun-small]
    S -->|успех| OK
    S -->|ошибка| ERR[Грейсфул-ошибка пользователю + лог]
