...
flowchart TB
    ...
    LLM[X5 CoPilot  внешний]
    GW -->|ТОЛЬКО обезличенный текст + TLS + corp CA| LLM
...
| **TLS с корпоративным CA** | как в `hh_analyser`: `LLM_CA_BUNDLE`, верификация **включена**. |
...
- **Секреты** (`API_KEY`, JWT-секрет, пароль БД) — в `.env`/секрет-хранилище, **не в коде/репозитории**; `.env.example` без значений (как в `hh_analyser`).
