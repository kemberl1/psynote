...
- Страница `/diary/batch`: дата поступления + период → сжатый опросник → пакетная генерация (дни 10/20/30… → `exam_10d` автоматически).
...
- Генерация `.docx` из текста дневника.
...
- Кнопки «Копировать / Word».

**Зачем:** довести результат до пригодного к подшивке документа.

**Проверка:** скачанный Word корректно открывается, форматирование сохранено, плейсхолдеры заменены на введённые значения.
